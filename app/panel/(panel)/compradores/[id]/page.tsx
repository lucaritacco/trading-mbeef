...
export default async function CompradorDetalle({
  ...
  return (
    <div className="max-w-3xl">
      <Link href="/panel/compradores" className="text-sm text-texto-sec transition-colors hover:text-texto">← Volver a compradores</Link>
      <h1 className="mt-4 font-serif text-3xl font-medium text-texto">{c.nombre}</h1>

      {error === "nombre" && (
        <p className="mt-6 border border-error/40 bg-error-suave px-4 py-3 text-sm text-error">El nombre es obligatorio.</p>
      )}
      {error && error !== "nombre" && (
      ...
      <form action={borrarComprador} className="mt-10 border-t border-borde pt-6">
        <input type="hidden" name="id" value={c.id} />
        <button type="submit" className="text-sm text-texto-sec transition-colors hover:text-primario">
          Eliminar comprador
        </button>
      </form>
    </div>
